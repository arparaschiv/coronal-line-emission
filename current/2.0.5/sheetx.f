...
C 
	SUBROUTINE SHEETX (RAD,THET,PH,B,T,P,EDENS,HDENS,
     *    TEMP,VEL,TURBV,IPRINT)
	INCLUDE 'PREC'
	PARAMETER(MCEN=10)
	DIMENSION XCEN(MCEN), YCEN(MCEN), ZCEN(MCEN), BCEN(MCEN)
	CHARACTER*(4) SKIND
	COMMON /CSHEETX/ ICALL,BS, TEMPS, VELS, TURBVS, SHEETNS,
     *                 SKIND,NCEN,XCEN,YCEN,ZCEN,BCEN
...
C
	   ...
	   IF(SKIND(1:4) .EQ. 'PLAS') THEN
	      ...
	      READ(LDUMS,*)(XCEN(J),J=1,NCEN)
	      WRITE(LOUT,710) 'X',(XCEN(J),J=1,NCEN)
...
C
	   D=DELTM
	   BX=0.  
	   AXP=  B*XLINPOT1(Y,Z+D) + DIPOT(DM,XM,YM,ZM,RX,RY,RZ+D)
	   BY =  (AXP-AX)/D
	   AXP = B*XLINPOT1(Y+D,Z) + DIPOT(DM,XM,YM,ZM,RX,RY+D,RZ)
	   BZ= -(AXP-AX)/D
	   T=ATAN2(BY,BZ)
	   P=PI/2.
...
C
	...
	DO J=1,N
	   ...
	   XLINPOT1= XLINPOT1 -BCEN(J)*DLOG(R)
	ENDDO
	RETURN
	END
