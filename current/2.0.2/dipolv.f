...
C 
	SUBROUTINE DIPOLV(RAD,THET,PH,B,T,P,EDENS,HDENS,
	...
	INCLUDE 'CINPUT'
	INCLUDE 'CLU'
	COMMON /CDIP/ XD,YD,ZD,XM,YM,ZM,DM,DELTA,BASE,V1,V2,T1,NTUBE
...
C	TEMP1=2.E6
	RHOM=(1.3625*0.8*UU)
	PTOT=BK*TEMP*1.88 + 0.5* RHOM*(TURBV*TURBV)*1.E10
	HSCALE = PTOT/RHOM /GRAV
...
C**********************************************************************
C
C	FUNCTION DIPOT(DM,XM,YM,ZM,RX,RY,RZ)
C	INCLUDE 'PREC'
C
C	R=SQRT(RX*RX + RY*RY + RZ*RZ)
C	DIPOT = -DM*RY/R/R/R
C	RETURN
C	END
C
C**********************************************************************
C
C	FUNCTION QUPOT(QM,XM,YM,ZM,RX,RY,RZ)
C	INCLUDE 'PREC'
C
C	R=SQRT(RX*RX + RY*RY + RZ*RZ)
C	C=RZ/R
C	QUPOT = QM* 0.5*(3.*C-1)/R/R/R
C	RETURN
C	END
